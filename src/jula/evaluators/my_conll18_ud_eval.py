...
def evaluate(gold_ud, system_ud):
    ...
    if gold_ud.characters != system_ud.characters:
        ...
        slice_ = slice(index, index + 20)
        raise UDError(
            "The concatenation of tokens in gold file and in system file differ!\n"
            + "First 20 differing characters in gold file: '{}' and system file: '{}'".format(
                "".join(map(_encode, gold_ud.characters[slice_])),
                "".join(map(_encode, system_ud.characters[slice_])),
            )
        )
...
def main(gold_lines, system_lines, verbose=False, counts=False):
    ...
    args = Args(gold_lines=gold_lines, system_lines=system_lines, verbose=verbose, counts=counts)  # FIX
