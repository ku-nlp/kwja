...
from jula.utils.constants import IGNORE_INDEX, NE_TAGS
...
class NERMetric(Metric):
    ...
    def compute(self) -> dict[str, Union[torch.Tensor, float]]:
        ...
        metrics = self.compute_ner_metrics(ne_tag_predictions, ne_tags)
        return metrics
    ...
    @staticmethod
    def align_predictions(prediction: np.ndarray, label: np.ndarray) -> tuple[list[list[str]], list[list[str]]]:
    ...
    def compute_ner_metrics(
        ...
        ner_metric = {}
        aligned_predictions, aligned_labels = self.align_predictions(
