...
class TypoModule(pl.LightningModule):
    ...
    def _truncate(batch: Any) -> None:
        ...
        for key, value in batch.items():
            if key in {"input_ids", "attention_mask", "kdr_labels", "ins_labels"}:
    ...
    def validation_step(self, batch: Any, batch_idx: int, dataloader_idx: Optional[int] = None) -> None:
        ...
        metric_args.update({"kdr_labels": batch["kdr_labels"], "ins_labels": batch["ins_labels"]})
        corpus = self.valid_corpora[dataloader_idx or 0]
        dataset = self.trainer.val_dataloaders[dataloader_idx or 0].dataset
        self.valid_corpus2typo_module_metric[corpus].update(metric_args, dataset)
    ...
    def test_step(self, batch: Any, batch_idx: int, dataloader_idx: Optional[int] = None) -> None:
        ...
        self.test_corpus2typo_module_metric[corpus].update(metric_args, dataset)
