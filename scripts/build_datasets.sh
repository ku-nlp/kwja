...
uv run python ./scripts/build_dataset.py "$WORK_DIR"/KWDLC/knp "$OUT_DIR"/kwdlc \
  --id "$WORK_DIR"/KWDLC/id/split_for_pas \
  --doc-id-format kwdlc \
  -j "$JOBS"
uv run python ./scripts/build_dataset.py "$WORK_DIR"/AnnotatedFKCCorpus/knp "$OUT_DIR"/fuman \
  --id "$WORK_DIR"/AnnotatedFKCCorpus/id/split_for_pas \
  -j "$JOBS"
uv run python ./scripts/build_dataset.py "$WORK_DIR"/WikipediaAnnotatedCorpus/knp "$OUT_DIR"/wac \
