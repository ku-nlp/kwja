...
grep '^EOS$' "$WORK_DIR/benchmark.jumanpp.juman" | wc -l > "$WORK_DIR/count.txt"
...
grep "# S-ID:" "$WORK_DIR/benchmark.ne.knp" | grep -v "ERROR:" | wc -l >> "$WORK_DIR/count.txt"

echo "Juman++ & KNP (固有表現認識・照応解析あり)"
# 解析が不安定なため、一文ずつ入力
(time (IFS=$'\n'; for sentence in $(cat "$INPUT" | tr '"' '\"'); do echo "${sentence}" | jumanpp | knp -tab -ne-crf -anaphora >> "$WORK_DIR/benchmark.ne_anaphora.knp"; done;)) 2>> "$WORK_DIR/benchmark.stderr"
grep "# S-ID:" "$WORK_DIR/benchmark.ne_anaphora.knp" | grep -v "ERROR:" | wc -l >> "$WORK_DIR/count.txt"
